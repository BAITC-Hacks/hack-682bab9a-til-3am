...
import { catalog, confirmConfirmation, createConfirmation, getCart, live, sendMessage } from './api';
import type { BackendCart, Confirmation, Product } from './types';
import './style.css';
import './cart.css';
...
function ProductCard({ product, onPrepare, pending, onConfirm, confirming }: { product: Product; onPrepare: (product: Product) => void; pending: Confirmation | null; onConfirm: () => void; confirming: boolean }) {
  ...
  const isPending = pending?.items.some(item => item.product_id === String(product.id));
...
function App() {
  ...
  async function prepare(product: Product) {
    setError('');
    try { setPending(await createConfirmation(product)); }
    catch (cause) { setError(cause instanceof Error ? cause.message : 'Не удалось подготовить добавление.'); }
  }
