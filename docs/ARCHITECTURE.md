...
Прототип реализован на React + TypeScript (чат) и FastAPI (API и логика ассистента). ИИ-логика — отдельный модуль backend-приложения, а не самостоятельный сетевой сервис: это упрощает локальный запуск. LLM (Google Gemini через OpenAI-совместимый API) используется только для разбора свободного текста и ответа на казахском; факты берутся из каталога. Инструкция запуска и сценарии проверки — в [README](../README.md).
...
| `POST /api/v1/sessions/{session_id}/confirmations` | Проверить позиции и создать одноразовое подтверждение добавления в демо-корзину. |
| `POST /api/v1/sessions/{session_id}/confirmations/{confirmation_id}/confirm` | Повторно проверить остатки и применить подтверждённые позиции к демо-корзине. |
